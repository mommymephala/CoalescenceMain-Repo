...
namespace AeLa.EasyFeedback.APIs
{
	public class Trello
	{
		...
		public string GetURI(string apiPath)
		{
			...
			return $"{ApiUri}{apiPath}{ext}key={AppKey}&token={token}";
		}
		...
		public static bool IsValidToken(string token, bool silent = false)
		{
			...
			// (GET /1/members/me)
			var requestUrl =
				$"{ApiUri}/members/me?key={AppKey}&token={token}";
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		)
		{
			...
			{
				Debug.LogError(
					$"Card description length is higher than maximum length of {MaxCharLength}. Truncating..."
				);
				description = description.Remove(MaxCharLength - 1);
			}
